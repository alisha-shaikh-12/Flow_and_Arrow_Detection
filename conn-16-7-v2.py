...
def build_connector_record(idx, tip_cand, companion, gray, img, cfg, page_size,
    ...
    if label:
        label_direction = "inflow" if label["direction_word"] == "FROM" else "outflow"
    ...
    shape_direction = "inflow" if tip_side == "left" else ("outflow" if tip_side == "right" else None)
    ...
    # direction_source is a compound, self-describing string:
    #   "caption:from_to_label"        -- parsed from a FROM/TO caption
    #   "geometry:tip_point"           -- inferred from the triangular tip only
    #   "caption:from_to_label+geometry:tip_point:agree"    -- both signals, matching
    #   "caption:from_to_label+geometry:tip_point:conflict" -- both signals, disagree
    if label_direction is not None and shape_direction is not None:
        direction_source = (
            "caption:from_to_label+geometry:tip_point:agree" if direction_agrees
            else "caption:from_to_label+geometry:tip_point:conflict"
        )
    elif label_direction is not None:
        direction_source = "caption:from_to_label"
    elif shape_direction is not None:
        direction_source = "geometry:tip_point"
    else:
    ...
    # raw_ocr_text keeps every OCR pass verbatim (tip cell / companion cell /
    # caption band) newline-separated, so nothing tesseract produced is lost
    # even when the downstream regex parsing below fails or mis-parses.
    raw_ocr_parts = [t for t in (tip_text, companion_text, label_raw.strip()) if t]
    raw_ocr_text = "\n".join(raw_ocr_parts) or None
    ...
    record = {
        ...
        "page": page_num,
        "bbox_xywh": [int(x0), int(y0), int(w0), int(h0)],
        "geometry": {
        ...
        # reference_tag: the sister-drawing/line number this connector points
        # to (e.g. "F-140"). sheet_grid_reference: the companion cell (e.g.
        # the sheet grid ref or a plain numeric id like "6544"/"1011").
        "reference_tag": target_drawing_number or None,
        "reference_tag_matches_drawing_series": strict_semantic_match,
        "reference_tag_ocr_confidence": round(target_conf, 1),
        "sheet_grid_reference": sheet_grid_reference or None,
        "sheet_grid_reference_ocr_confidence": round(grid_conf, 1),
        # description: the raw FROM/TO caption text as OCR'd, before regex
        # parsing -- kept even when parsing fails so a human reviewer (or a
        # downstream re-parse pass) still has something to work with.
        "description": label_raw.strip() or None,
        "equipment_tag": connected_equipment,
        "raw_ocr_text": raw_ocr_text,
        "graph_edge": {
            "from_node": (
                f"{target_drawing_number}:{connected_equipment}" if direction == "inflow" and target_drawing_number
                else f"{drawing_number}:conn_{page_num:02d}_{idx:04d}"
            ),
            "to_node": (
                f"{drawing_number}:conn_{page_num:02d}_{idx:04d}" if direction == "inflow"
    ...
    if not target_drawing_number:
        record["flags"].append("reference_tag_ocr_empty")
...
def _load_annotation_font(size):
    import PIL.ImageFont as ImageFont

    # Try a handful of common truetype fonts; fall back to PIL's default
    # bitmap font (which ignores `size`) only if none are available.
    for candidate in (
        "DejaVuSans-Bold.ttf",
...
def annotate(img: Image.Image, records, out_path, font_size=28):
    ...
    for r in records:
        ...
        is_inflow = r["direction"] == "inflow"
        color = (0, 140, 0) if is_inflow else (200, 0, 0)
        draw.rectangle([x0 - 6, y0 - 6, x1 + 6, y1 + 6], outline=color, width=6)

        label = f'{r["id"]} {r["reference_tag"]}/{r["sheet_grid_reference"]} {r["direction"]}'
        text_xy = (x0, y1 + 10)
        # Measure the text so the background box fits it exactly, then pad
        # it out a bit -- a solid bg is what makes the label legible at
        # normal zoom instead of disappearing into the line-art.
...
def _looks_like_real_connector(rec):
    ...
    if rec["equipment_tag"]:
        return True
    if rec["reference_tag"] and rec.get("reference_tag_matches_drawing_series"):
        return True
    return False
...
def process_page(page_png: Path, page_num: int, cfg, drawing_number, source_file, out_dir, line_prefix):
    ...
    annotate(img_rgb, kept, out_dir / f"{stem}_connectors_annotated_p{page_num}.png")
    return kept, dropped
...
def main():
    ...
    ap.add_argument("--out", default="connector_output",
                     help="Base output folder. Each run gets its own subfolder named after "
                          "the input file, so re-running on the same or a different PDF "
    ...
    # Per-run subfolder named after the input file. If a folder with that
    # name already exists (e.g. this exact PDF was processed before), append
    # a run timestamp instead of clobbering the earlier output.
    out_dir = base_out / in_path.stem
    if out_dir.exists():
        out_dir = base_out / f"{in_path.stem}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    out_json = out_dir / f"{in_path.stem}_connectors.json"
    payload = {
        "drawing_number": drawing_number,
